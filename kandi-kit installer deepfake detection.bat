...
ECHO======================================================================================
SET KIT_NAME=deepfake-detection
...
SET REPO_DOWNLOAD_URL=https://github.com/kandi1clickkits/deepfake-detection/releases/download/v1.0.0/deepfake-detection.zip
SET REPO_DEPENDENCIES_URL=https://github.com/kandi1clickkits/deepfake-detection/raw/main/requirements.txt
SET REPO_NAME=deepfake-detection.zip
SET EXTRACTED_REPO_DIR=deepfake-detection
SET NOTEBOOK_NAME=Deepfake_detection.ipynb
SET MS_VC_REDIST_URL=https://aka.ms/vs/17/release/vs_BuildTools.exe
SET GET_PIP_LOCATION=https://bootstrap.pypa.io/get-pip.py
SET VIRTUALENV_NAME=deepfake-detection-env
...
IF EXIST !WORKING_DIR!\%EXTRACTED_REPO_DIR%\ (
	...
	CALL :Download_model_checkpoint
    	TITLE Installing %KIT_NAME% kit 100%% xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx
    	timeout 1  >nul
	for /f %%A in ('copy /Z "%~dpf0" nul') do set "CR=%%A"
	<nul set/p"=->!CR!"
...
CALL :LOG "Downloading M source ... "
cd %EXTRACTED_REPO_DIR%
curl https://kandi.dev/owassets/kandi1clickkits-model-assets/resnetinceptionv1_epoch_32.pth --output resnetinceptionv1_epoch_32.pth >> !WORKING_DIR!\log.txt 2>&1
IF ERRORLEVEL 1 (
...
cd ..
CALL :LOG "Model weights/checkpoint file downloaded from source"
EXIT /B 0
